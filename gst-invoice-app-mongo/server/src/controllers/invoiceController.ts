...
export const createInvoice: RequestHandler = async (req, res) => {
    ...
    const clientExists = await Client.findOne({ _id: clientId, business: businessId });
        if (!clientExists) {
        ...
        const newInvoice = new Invoice({
            ...
            client: clientId,
            items,
            business: businessId, // Link to the business from middleware
        });
...
export const getInvoiceSummary: RequestHandler = async (req, res) => {
    const { businessId } = (req as AuthRequest);
    try {
        const stats = await Invoice.aggregate([
            { $match: { business: new mongoose.Types.ObjectId(businessId) } }, // Match business
            {
                $group: {
...
export const updateOverdueInvoices: RequestHandler = async (req, res): Promise<void> => {
    try {
